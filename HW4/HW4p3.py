...
def f(x):
    theta = np.array([-30,0,30])/180.0*math.pi  # radians
    R = np.array([6870,6728,6615])   #given  
    f = np.zeros(len(x))   #p. 166 text
    f[0] = x[1]/(1+x[2] * math.sin(theta[0] + x[0]))-R[0]
    f[1] = x[1]/(1+x[2]* math.sin(theta[1] + x[0]))-R[1]
    f[2] = x[1]/(1+x[2] * math.sin(theta[2] + x[0]))-R[2]  
    return f
...
def main():
    ...
    x_init=np.array([1,8000,.1])
    
    root = newtonRaphson2(f,x_init)
    
    print('alpha='+str(root[0]))
    print('C='+str(root[1]))
    print('e='+str(root[2]))
main()
